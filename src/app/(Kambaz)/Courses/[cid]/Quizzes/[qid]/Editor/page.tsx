...
export default function QuizEditor() {
  ...
  React.useEffect(() => {
    if (!isNew && existingquiz) {
      ...
      if (
        ...
        Array.isArray(quizWithQuestions.questions)
      ) {
        const needsMigration = quizWithQuestions.questions.some(
          (q: any) =>
            q.type === "FILL_IN_BLANK" && q.possibleAnswers && !q.blanks
        );

        if (needsMigration) {
          const migratedQuestions = quizWithQuestions.questions.map(
            (q: any) => {
              if (
  ...
  if (!isNew && !quiz) {
    return (
      <div className="text-danger">
        Quiz with ID **{qid}** not found in the database.
      </div>
    );
  }
  ...
  const handleSave = async (Publish = false) => {
    ...
    for (let i = 0; i < quiz.questions.length; i++) {
      ...
      if (
        ...
        const emptyChoice = question.choices.find(
          (choice: any) => !choice.text.trim()
        );
        if (emptyChoice) {
        ...
        if (correctAnswers.length === 0) {
          alert(
            `Question ${i + 1}: Please mark at least one choice as correct`
          );
          return;
        }
      ...
      if (question.type === "FILL_IN_BLANK") {
        ...
        for (let j = 0; j < question.blanks.length; j++) {
          ...
          if (
            ...
            blank.possibleAnswers.length === 0 ||
  ...
  const updateChoice = (
    ...
    updatedQuestions[questionIndex] = {
      ...updatedQuestions[questionIndex],
      choices: updatedQuestions[questionIndex].choices.map(
  ...
  return (
    <div id="wd-quizzes-editor">
      ...
      {activeTab === "details" && (
        <>
          <FormGroup className="mb-3">
            <FormLabel htmlFor="title" className="fw-bold">
              Quiz Name
            </FormLabel>
            <FormControl
              id="title"
          ...
          <Form>
            <FormGroup as={Row} className="align-items-center">
              <FormLabel column sm="3" className="text-end fw-bold">
                Quiz Type
              </FormLabel>
              <Col>
                <FormSelect
              ...
              <Col>
                <FormSelect
                  ...
                  onChange={(e) =>
                    setQuiz({
                      ...quiz,
                      shuffleAnswers: e.target.value === "true",
                    })
                  }
                  disabled={!isFaculty}
                >
              ...
              <Col>
                <FormSelect
                  ...
                  onChange={(e) =>
                    setQuiz({
                      ...quiz,
                      multipleAttempts: e.target.value === "true",
                    })
                  }
                  disabled={!isFaculty}
                >
              ...
              <Col>
                <FormSelect
                  ...
                  onChange={(e) =>
                    setQuiz({
                      ...quiz,
                      oneQuestionAtTime: e.target.value === "true",
                    })
                  }
                  disabled={!isFaculty}
                >
              ...
              <Col>
                <FormSelect
                  ...
                  onChange={(e) =>
                    setQuiz({
                      ...quiz,
                      webcamRequired: e.target.value === "true",
                    })
                  }
                  disabled={!isFaculty}
                >
              ...
              <Col>
                <FormSelect
                  ...
                  onChange={(e) =>
                    setQuiz({
                      ...quiz,
                      lockQuestionsAfterAnswering: e.target.value === "true",
                    })
                  }
                  disabled={!isFaculty}
                >
                  ...
                  <Row>
                    <Col sm={6}>
                      <FormGroup className="mb-2">
                        <FormLabel htmlFor="availDate" className="fw-bold">
                          Available from
                        </FormLabel>
                    ...
                    <Col sm={6}>
                      <FormGroup className="mb-3">
                        <FormLabel htmlFor="dueDate" className="fw-bold">
                          Until
                        </FormLabel>
                        <div className="d-flex align-items-center position-relative">
                          <FormControl
              ...
              {question.type === "TRUE_FALSE" && (
                <div>
                  ...
                  <FormCheck
                    ...
                    onChange={() =>
                      handleQuestionChange(qIndex, "correctAnswer", true)
                    }
                  />
                  <FormCheck
                    type="radio"
                    ...
                    name={`trueFalse-${qIndex}`}
                    onChange={() =>
                      handleQuestionChange(qIndex, "correctAnswer", false)
                    }
                  />
                </div>
              )}
                      ...
                      >
                        ...
                        <FormGroup>
                          ...
                          <FormControl
                            ...
                            onChange={(e) =>
                              updateBlankAnswers(qIndex, bIndex, e.target.value)
                            }
                          />
      ...
      <div className="d-flex justify-content-end">
        ...
        {!isFaculty && (
          <Link href={`/Courses/${cid}/Quizzes`}>
            <Button id="wd-cancel-btn" variant="light" className="border me-2">
              Back
            </Button>
          </Link>
        )}
